				...
				<div className="accordion-item" key = {Math.random()}>
					...
					<div id={item.id_two} className={`accordion-collapse collapse ${item.show ? 'show': ''}`} aria-labelledby={item.aria_labelledby} data-bs-parent="#accordionExample">
  						<div className="accordion-body">
    						<strong className = 'text-success'>PETx</strong> is a platform that enable user to access major pet related services from a single platform. PETx is trying to build a better community for the animals by providing emergency services, medical services, online shopping center and other services
  						</div>
					</div>
				</div>
			)
